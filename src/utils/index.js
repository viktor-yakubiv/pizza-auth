...
export const bindAll = (context, ...names) => {
  names.forEach((name) => {
    if (typeof context[name] === 'function') {
      ...
      throw Error(`Expected function ${name}. Instead received: ${typeof context[name]}`)
    }
  })
}

export const getMidnightWeather = list =>
  list.filter(({ dt_txt: date }) => date.includes(MIDNIGHT_HOURS))
...
RequestError.prototype.toString = function toRequestErrorString() {
